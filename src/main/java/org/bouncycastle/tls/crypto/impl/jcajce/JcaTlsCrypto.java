    ...
    extends AbstractTlsCrypto
{
    ...
    {
        // by SK
        if (namedGroup==NamedGroup.oqs_p256_frodo640aes)
            return null; // no disabled algorithms

        if (namedGroup==NamedGroup.oqs_frodo640aes)
            return null; // no disabled algorithms

        if (namedGroup==NamedGroup.oqs_frodo640shake)
            return null; // no disabled algorithms
...
//        case SignatureAlgorithm.sm2:
    ...
    public boolean hasSignatureScheme(int signatureScheme)
    {
        ...
        {
            case SignatureScheme.oqs_rainbowIclassic: // by SK
            case SignatureScheme.oqs_sphincsshake256128frobust: // by SK2
                return true;
...
//            }
    ...
    protected Boolean isSupportedNamedGroup(int namedGroup)
    {
        try
        {
            if (namedGroup == NamedGroup.oqs_p256_frodo640aes) { // by SK
                return true; // by SK
            }
            if (namedGroup == NamedGroup.oqs_frodo640aes) { // by SK
                return true; // by SK
            }
            if (NamedGroup.refersToAnXDHCurve(namedGroup))
            {
