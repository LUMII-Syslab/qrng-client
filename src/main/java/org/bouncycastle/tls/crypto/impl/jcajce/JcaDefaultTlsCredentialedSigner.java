...
import org.bouncycastle.pqc.crypto.sphincsplus.SPHINCSPlusSigner; // by SK client-side
/**
    ...
    extends DefaultTlsCredentialedSigner
{
        ...
        SignatureAndHashAlgorithm signatureAndHashAlgorithm)
    {
        ...
        else if ("SPHINCS+".equalsIgnoreCase(algorithm))// by SK client cert
        {
            signer = new SPHINCSPlusSigner();
            assert (privateKey instanceof BCSPHINCSPlusPrivateKey);
            BCSPHINCSPlusPrivateKey k = (BCSPHINCSPlusPrivateKey)privateKey;
            SPHINCSPlusPrivateKeyParameters p = k.getPrivateKeyParams();
            ((SPHINCSPlusSigner)signer).initForSigning(p);
        }
        else
        {
