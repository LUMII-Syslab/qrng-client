    ...
    */
    ...
    private void loadAlgorithms(String packageName, String[] names)
    {
        for (int i = 0; i != names.length; i++)
        {
            ...
            // by SK: (reflection workaround for GraalVM NativeImage
            if ("SPHINCSPlus".equals(names[i])) {
                new SPHINCSPlus.Mappings().configure(this);
                return;
            }
            else if ("Frodo".equals(names[i])) {
                new Frodo.Mappings().configure(this);
                return;
            }
            //^^^ by SK
