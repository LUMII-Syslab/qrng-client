...
public class PublicKeyFactory
{
    ...
    static
    {
        ...
        converters.put(PQCObjectIdentifiers.oqs_sphincsshake256128frobust, new SPHINCSPlusConverter()); // by SK2
        ...
        extends SubjectPublicKeyInfoConverter
    {
        ...
        {
            // by SK2
            byte[] keyEnc = keyInfo.octets(); // ASN1OctetString.getInstance(keyInfo.parsePublicKey(true)).getOctets();

            AlgorithmIdentifier alg = keyInfo.getAlgorithm();//.toString();//.getParameters();
            byte[] b = alg.getEncoded();
            /*if (alg.equals("1.3.9999.6.7.1")) {*/
...
//                return new SPHINCSPlusPublicKeyParameters(spParams, Arrays.copyOfRange(keyEnc, 4, keyEnc.length));
            ...
            //throw new RuntimeException("unknown sphincs+ variant");
        }
    }
