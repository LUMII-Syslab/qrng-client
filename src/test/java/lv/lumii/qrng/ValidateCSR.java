...
public class ValidateCSR {
    ...
    public static void main(String[] args) throws Exception {
        ...
        SubjectPublicKeyInfo pk = SubjectPublicKeyInfo.getInstance(whatToSign.getObjectAt(2));

        ASN1Sequence csrAlgorithmSeq = (ASN1Sequence)csrSeq.getObjectAt(1);
        ASN1ObjectIdentifier csrAlgorithmOid = (ASN1ObjectIdentifier)csrAlgorithmSeq.getObjectAt(0);
        ASN1BitString csrSignature = (ASN1BitString)csrSeq.getObjectAt(2);

        org.bouncycastle.openssl.jcajce.JcaPEMKeyConverter converter = new org.bouncycastle.openssl.jcajce.JcaPEMKeyConverter();
        PublicKey publicKey2 = converter.getPublicKey(pk);

        org.bouncycastle.operator.ContentVerifier contentVerifier = null;
        try {
            AlgorithmIdentifier algId = new AlgorithmIdentifier(csrAlgorithmOid);//AlgorithmIdentifier.getInstance(csrAlgorithmOid);
            contentVerifier = new org.bouncycastle.operator.jcajce.JcaContentVerifierProviderBuilder().build(publicKey2).get(
                    algId
            );
        } catch (Exception e) {
            //e.printStackTrace();
        ...
        OutputStream sOut = contentVerifier.getOutputStream();

        sOut.write(whatToSign.getEncoded());

        sOut.close();

        boolean b = contentVerifier.verify(csrSignature.getBytes());
        System.out.println("VERIFY RETURNED "+b);
    }
